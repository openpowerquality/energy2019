...
\title{\textbf{\Large A Transient Classification System Implementation on an Open Source Distributed Power Quality Network}\\[0.2ex]}
...
Capturing and classifying power quality phenomena is a primary concern in the energy fields. Logging power quality events and identifying recurring issues is necessary for the development of an efficient and stable grid.  

This paper presents methods for classifying transients recorded on an active open source network of power quality sensors. The methods follows a tractable algorithm which applies well understood signal processing methods and statistical inference for feature extraction and decision making.

The methods proposed are tested on multiple types of simulated power quality disturbances demonstrating the capabilities of the system. The results of this research leads to the successful implementation of a transient classifier on an open source distributed power quality network, OpenPowerQuality. Additional functionality can be easily added to the system, including a meta analysis to capture higher level network wide events on the the OpenPowerQuality network.
